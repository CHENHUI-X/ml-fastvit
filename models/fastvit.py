...
def convolutional_stem(
    ...
    return nn.Sequential(
        MobileOneBlock(
            ...
            use_se=False,
            num_conv_branches=1,
        ), #  Stem中的第一个 block : 这个输入C 和输出C 是不一样的
        MobileOneBlock(
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        block = list()
        block.append(
            # patch emb 的上半部分, 一个 大核 + 一个小核 , stride = 2 进行下采样
            ReparamLargeKernelConv(
                in_channels=in_channels,
                out_channels=embed_dim,
                kernel_size=patch_size,
                stride=stride, # paper 中 stride = 2 
                groups=in_channels,
                small_kernel=3,
                inference_mode=inference_mode,
            )
        ...
        block.append(
            # patch embed 的下半部分 一个 BN + 一个 1 * 1 的conv , 所以 kernel size  = 1 
            MobileOneBlock(
...
class RepMixer(nn.Module):
    ...
    def __init__(
        ...
        if inference_mode:
            # inference的时候 , 直接定义好尺寸即可, 直接load weight parameter that has re - parameter 
            ...
            # mixer 返回 BN 层   + 1 * 1 scale层 + DW Conv 层
            # 二者相减 就把BN层抵消了 , 但是还有个中间的 1 * 1 好像没有在paper中的stage的结构中看到
            # FIXME: 中间的scale层没有在paper中的图示结构看到
